...
public class DashboardService {
    ...
    private List<SupervisorDashboardSummaryResponse.TrendPoint> getMonthlyAvgSalesChangeRateTrend(List<Long> storeIds) {
        ...
        Double prev = null;package com.franchise.backend.store.service;
...
import java.util.stream.Collectors;
        ...
        public class DashboardService {
            ...
            public List<StoreListResponse> getStoresForManager(String managerLoginId, StoreSearchRequest condition) {
                ...
                // 정렬
                com.franchise.backend.store.service.DashboardService.StoreSort sort = normalizeSort(condition.getSort());
                rows.sort(sort.getComparator());
            ...
            public SupervisorDashboardSummaryResponse getSupervisorSummary(String supervisorLoginId) {
                ...
                // 7) 방문 현황(이번 달): "이번 달 QSC COMPLETED"가 1건 이상 있으면 방문 완료로 간주
                com.franchise.backend.store.service.DashboardService.VisitAgg visitAgg = getVisitStatusByQscThisMonth(storeIds);
                int rate = (visitAgg.total == 0) ? 0 : (int) Math.round((visitAgg.completed * 100.0) / visitAgg.total);
        ...
        """;
            ...
            private com.franchise.backend.store.service.DashboardService.VisitAgg getVisitStatusByQscThisMonth(List<Long> storeIds) {
                if (storeIds == null || storeIds.isEmpty()) return new com.franchise.backend.store.service.DashboardService.VisitAgg(0, 0);
        ...
        """;
                ...
                try {
                    ...
                    return new com.franchise.backend.store.service.DashboardService.VisitAgg(visited, storeIds.size());
                } catch (DataAccessException e) {
                    return new com.franchise.backend.store.service.DashboardService.VisitAgg(0, storeIds.size());
                }
            }
        ...
        """;
            ...
            private com.franchise.backend.store.service.DashboardService.StoreSort normalizeSort(String sort) {
                if (sort == null || sort.isBlank()) return com.franchise.backend.store.service.DashboardService.StoreSort.INSPECTED_AT_DESC;
                try {
                    return com.franchise.backend.store.service.DashboardService.StoreSort.valueOf(sort.trim().toUpperCase());
                } catch (Exception e) {
                    return com.franchise.backend.store.service.DashboardService.StoreSort.INSPECTED_AT_DESC;
                }
            }
