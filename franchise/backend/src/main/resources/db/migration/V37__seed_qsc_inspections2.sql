/* =========================================================
   [Flyway/DEMO] 망원점(store_id=6) QSC 정기(REGULAR) 시연 데이터 생성
   - store_id=6 데이터만 삭제/재생성
   - inspector_id=9
   - 기간: 2025-06 ~ 2025-08 (총 6회, 월 2회)
   - 2025-08: 8/05, 8/20 고정 + 전부 불합격(<70)
   - qsc_master 컬럼: inspected_at 사용
   - qsc_master.total_score NOT NULL 대응: INSERT 시 0으로 선기입 후 UPDATE
   ========================================================= */
...
plan_with_tpl AS (
  SELECT
    ...
    (
      ...
      ORDER BY t.effective_from DESC, t.template_id DESC
      LIMIT 1
    ) AS template_id
  FROM plan p
),
...
targets AS (
  SELECT
    ...
    p.target_total_score,
    ic.n_items,
    LEAST(ic.n_items*5, GREATEST(0, ROUND(p.target_total_score * ic.n_items * 5.0 / 100.0)::int)) AS target_item_sum,
    m.created_at,
    m.updated_at
  FROM ins_master m
  JOIN plan_ts p
...
items_scored AS (
  SELECT
    ...
    LEAST(
      5,
      (target_item_sum / n_items) + CASE WHEN rn_item <= (target_item_sum % n_items) THEN 1 ELSE 0 END
    )::int AS score,
    created_at,
    updated_at
  FROM items_ranked
