...
import { MOCK_INSPECTIONS, FIXED_QSC_CATEGORIES, MOCK_TEMPLATES, getMockInspectionDetails, QSC_GRADE_CRITERIA } from '@/lib/mock/mockQscData';
import {
    Calendar, User, ArrowLeft, Download, ExternalLink,
    CheckCircle2, XCircle, Camera, MessageSquare, FileText
} from 'lucide-react';
...
export default function InspectionReportPage() {
    ...
    // Tab State
    const [activeTab, setActiveTab] = useState('quality'); // Default first tab
    ...
    const getScoreBadge = (score: number) => {
        if (score === 5) return <span className="text-green-600 font-bold text-sm bg-green-50 px-2 py-0.5 rounded border border-green-200">매우만족(5)</span>;
        if (score === 4) return <span className="text-blue-600 font-bold text-sm bg-blue-50 px-2 py-0.5 rounded border border-blue-200">만족(4)</span>;
        if (score === 3) return <span className="text-yellow-600 font-bold text-sm bg-yellow-50 px-2 py-0.5 rounded border border-yellow-200">보통(3)</span>;
        if (score <= 2) return <span className="text-red-600 font-bold text-sm bg-red-50 px-2 py-0.5 rounded border border-red-200">미흡({score})</span>;
        return <span>-</span>;
    };
