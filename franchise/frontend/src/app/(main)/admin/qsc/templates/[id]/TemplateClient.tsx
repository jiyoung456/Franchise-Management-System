...
export default function TemplateClient({ id }: { id: string }) {
    ...
    const [isReadOnly, setIsReadOnly] = useState(id !== 'new');
                            ...
                            <div className="space-y-2">
                                {items.filter(i => (i.subcategory || '기타') === sub).map((item, idx) => (
                                    <div key={item.templateItemId} className="flex items-center gap-4 py-3 px-4 bg-white rounded-xl border border-gray-50 group hover:border-blue-100 transition-all shadow-sm">
                                        ...
                                        {isReadOnly ? (
                                            <span className="flex-1 text-sm font-bold text-gray-700">{item.itemName}</span>
                                        ) : (
                                            <input
                                                type="text"
                                                value={item.itemName}
                                        ...
                                        <div className="flex items-center gap-3">
                                            <button
                                                onClick={() => !isReadOnly && handleUpdateItem(item.templateItemId, 'isRequired', !item.isRequired)}
                                                className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-md transition-all ${item.isRequired ? 'bg-red-50 text-red-500' : 'bg-gray-100 text-gray-400'} ${isReadOnly ? 'cursor-default' : ''}`}
                                            >
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 pb-20 mt-4">
            ...
            <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm space-y-6 mb-10">
                <div className="grid grid-cols-12 gap-x-6 gap-y-6">
                    <div className="col-span-12 lg:col-span-6">
                        ...
                        {isReadOnly ? (
                            <div className="w-full px-5 py-3.5 bg-gray-50/50 rounded-xl text-sm font-bold text-gray-900 border border-transparent">
                                {form.templateName}
                            </div>
                        ) : (
                            <input
                    ...
                    <div className="col-span-12 lg:col-span-3">
                        ...
                        {isReadOnly ? (
                            <div className="w-full px-5 py-3.5 bg-gray-50/50 rounded-xl text-sm font-bold text-gray-900 border border-transparent">
                                {form.inspectionType === 'REGULAR' ? '정기 점검' : form.inspectionType === 'SPECIAL' ? '특별 점검' : form.inspectionType === 'REINSPECTION' ? '재점검' : '선택 중'}
                            </div>
                        ) : (
                            <select
                    ...
                    <div className="col-span-12 lg:col-span-3">
                        ...
                        <div className="h-[46px] flex items-center px-2">
                            {isReadOnly ? (
                                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${form.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                    {form.status === 'ACTIVE' ? '활성 (Active)' : '비활성 (Inactive)'}
                                </span>
                            ) : (
            ...
            <div className="space-y-10">
                ...
                {renderCategoryBlock(FIXED_QSC_CATEGORIES.safety)}
            </div>
        </div >
    );
}
